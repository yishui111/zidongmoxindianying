# 风之原 · 二次元开放世界（可玩原型 v0.4）

> 目标：做一款《原神》方向的二次元开放世界游戏。当前版本是**可玩的架构底座**：
> 支持自定义 VRM 角色导入、多场景切换、动作状态机、攀爬、滑翔、游泳、昼夜循环、
> 收集任务、**战斗系统**（怪物 AI / 挥剑 / 伤害数字 / 掉落 / 死亡复活）。
...
│   ├─ quest    收集任务（完成时广播 quest:complete）
│   ├─ combat   战斗：史莱姆 AI/挥剑判定/伤害数字/掉落/重生
│   └─ effects  风元素光尘/烟花（监听 quest:complete 放烟花）
...
**一键启动**：双击 `anime-open-world/启动游戏.bat` —— 自动启动服务器（最小化窗口）并打开浏览器。
**一键关闭**：双击 `关闭游戏.bat`。
（服务器已在运行时，启动脚本只会打开游戏页面，不会重复开服务器。）
...
**左键/J 攻击**（3.5m 自动索敌）· **F 开宝箱** ·
`C`/数字键 切换角色 · 鼠标转视角（点击画面锁定，Esc释放） · 滚轮缩放 · `H` 提示 · `M` 静音/音乐
...
│   │   ├── combat.js       #   战斗：史莱姆 AI/挥剑判定/伤害数字/掉落/重生
│   │   └── effects.js      #   风元素光尘/烟花
...
- 怪物：史莱姆（游荡 → 玩家靠近 11m 追击 → **下蹲蓄力前摇** → 撞击造成伤害），头顶血条，死亡后 8 秒换位重生
- 玩家：左键/J 攻击（3.5m 自动索敌、两段挥砍、伤害数字、击退）；血条归零后在祭坛复苏（3 秒无敌）
- 掉落：40% 概率回复球（踩到 +25 HP）
- 数量配置：`scene.json → "combat": { "enemies": 8 }`
- 代码位置：怪物/AI/判定/掉落全部在 `js/modules/combat.js`，玩家血量结算在 `player.js`（事件总线解耦）

## 宝箱与探索

- 全岛散布可开启宝箱（海岛 5 个 / 山谷 6 个），靠近时高亮提示，按 `F` 开启
- 随机奖励：生命上限 +10 / 体力回满 / 完全回复
- 开盖动画 + 音效 + 小地图位置，HUD 显示收集进度（`⚔ 击杀　🎁 宝箱`）
- 数量配置：`scene.json → "chests": { "count": 5 }`；代码 `js/modules/chests.js`

## 环境与氛围

- 程序化环境 BGM：和弦垫 + 五声琶音（`M` 键静音）
- 昼夜循环带**太阳/月亮天体**、星空、云层
- 草地与树冠风摆 shader、风元素光尘、水面波浪
- **场景装饰**（`js/world/decor.js`）：山崖瀑布（动态水流 + 底部雾气）、
  祭坛周边樱花林、高山松林、发光水晶矿簇、石板路（出生点→鸟居→祭坛）、
  山谷雾气、岸外浪涌带——全部程序化生成

## 传送锚点

- 全岛 3 个锚点：靠近按 `F` 激活（晶体由灰变蓝、光柱点亮）
- 按 `V` 打开传送面板，点击已激活锚点立即传送
- 代码：`js/modules/waypoints.js`（小地图蓝/灰菱形标记同步）
